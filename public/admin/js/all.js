...
;(function($, undefined) {
    ...
    function Modal(options) {
        ...
        this.$modal = $(this.options.target).attr('class', 'scomodal fade').hide();
        var self = this;
    ...
    $.extend(Modal.prototype, {
        show: function() {
            ...
            if (!this.$modal.length) {
                this.$modal = $('<div class="scomodal fade" id="' + this.options.target.substr(1) + '"><div class="scomodal-header"><a class="close" href="#" data-dismiss="scomodal">×</a><h3>&nbsp;</h3></div><div class="inner"/></div>').appendTo(this.options.appendTo).hide();
            }

            this.$modal.find('.modal-header h3').html(this.options.title);

            if (this.options.cssclass !== undefined) {
                this.$modal.attr('class', 'scomodal fade ' + this.options.cssclass);
            }
...
//            alert(typeof this.options.onClose);
    ...
    $(document).on('click.' + pluginName, '[data-trigger="scomodal"]', function() {
        $(this)[pluginName]();
        if ($(this).is('a')) {
            return false;
        }
    }).on('click.' + pluginName, '[data-dismiss="scomodal"]', function(e) {
        e.preventDefault();
        $(this).closest('.scomodal').trigger('close');
//        $(this).closest('.scomodal').trigger('destroy');
    });
})(jQuery);
/* ==========================================================
 * sco.confirm.js
...
//		alert(this instanceof Confirm);
		if (!$modal.length) {
			$modal = $('<div class="scomodal" id="' + this.options.target.substr(1) + '"><div class="scomodal-body inner"/><div class="scomodal-footer"><a class="btn cancel" href="#" data-dismiss="scomodal">'+ this.options.noBtn +'</a>&nbsp;&nbsp;&nbsp; <a href="#" class="btn btn-danger" data-action="1">'+ this.options.yesBtn +'</a></div></div>').appendTo(this.options.appendTo).hide();
		}
//			if (typeof this.options.callback !== 'undefined') {
//				var self = this;
//				$modal.find('[data-callback]').attr('href', 'javscript:void(0);').on('click.' + pluginName, function(e) {
//					e.preventDefault();
//					alert(self.options.callback);
...
//			}
		...
		if (typeof this.options.action == 'function') {
			var self = this;
			$modal.find('[data-action]').attr('href', '#').unbind().on('click.' + pluginName, function(e) {
				e.preventDefault();
	...
	$.fn[pluginName].defaults = {
		...
		,yesBtn : '是'
		,noBtn : '否'
		,onClose :function(){$('[data-delmark="1"]').removeAttr('data-delmark');}
	};
        ...
        $(document).on('click', selector, function()
        {
            if(confirm(v.lang.confirmDelete))
            {
                ...
                $.getJSON(deleter.attr('href'), function(data)
                {
                    ...
                    {
                        alert(data.message);
                        location.reload();//tuyi 2014.12.11
                    }
                });
            }
            return false;
...
function selectLang(lang)
{
	...
	path = config.domain.substring(config.domain.indexOf('/') + 1 , config.domain.length);
	$.cookie('lang', lang, {expires:config.cookieLife, path: '/' + path + '/'});
    //location.href = removeAnchor(location.href);
    location.href = '/' + path + '/index.php/' + config.runMode + '/';
}
