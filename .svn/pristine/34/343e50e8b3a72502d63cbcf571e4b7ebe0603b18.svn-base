<?php
class clsInformationWaterqualityEditController extends clsAppController implements IAction_default , IAction_update {
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'waterquality')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->waterquality->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->waterquality->successupdate;
