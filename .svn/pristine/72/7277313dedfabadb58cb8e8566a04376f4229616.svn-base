<?php
class clsExampleSurfaceEditController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_category->update($input)) {
        	$this->output->result  = 'fail';
        	$this->output->message = $this->lang->surface->failupdate;
        	return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->surface->successupdate;
    ...
    private function init() {
        ...
        $model = new clsModModel($this->mdb, 'mw_category');
        $surface = $model->mw_category->getByID($input);
