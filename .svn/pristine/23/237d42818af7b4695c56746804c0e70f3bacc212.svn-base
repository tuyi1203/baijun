<?php
class clsServiceUsernoticeDefaultController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_single->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->usernotice->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->usernotice->successupdate;
        $this->output->locate  = U('service/usernotice');
    }
