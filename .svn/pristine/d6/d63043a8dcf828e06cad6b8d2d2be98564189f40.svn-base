<?php
class clsArticleSignEditController extends clsAppController implements IAction_default , IAction_update , IAction_publish{
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'sign')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->sign->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->sign->successupdate;
        $this->output->locate  = 'article_sign-default-default.html';
