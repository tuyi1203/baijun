<?php
class Yjm_Booking extends clsModel{

    CONST INSERT     = "insert into yjm_booking (name,tel,content,content2,createtime)values (?,?,?,?,?)";

    CONST GETLIST    = "select * from yjm_booking ";

    CONST GETCOUNT   = "select count(*) cnt from yjm_booking";

    CONST GET        = "select * from yjm_booking where id=?";

    CONST UPDATE     = "update yjm_booking set ";

    CONST DELETE     = "delete from yjm_booking where id =?";

    CONST PUBLISH    = "update yjm_booking set public=? where id=?";
