<?php
class clsAlbumSampleListController extends clsAppController implements IAction_default {
    ...
    public function _default() {
        ...
        $input->subkey = '2';
        $recTotal = $model->mw_album->getCount($input);
        $recperpage = 6 ;
        ...
        if ($recTotal > 0) {
			...
			$list = array();
			$albumlist = $model->mw_album->getList($input);
    ...
    public function _paging() {
        ...
        if ($recTotal > 0) {
            ...
            $input->start = $pager->getRecStart();
            $input->end   = $pager->getRecEnd();
//             $result = $model->mw_album->getImageList($input);
			$list = array();
			$albumlist = $model->mw_album->getList($input);
			foreach ($albumlist as $value) {
				$list[$value['id']]['albumtitle'] = $value['title'];
