<?php
class clsServiceWaterpriceDefaultController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_single->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->waterprice->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->waterprice->successupdate;
        $this->output->locate  = U('service/waterprice');
    }
