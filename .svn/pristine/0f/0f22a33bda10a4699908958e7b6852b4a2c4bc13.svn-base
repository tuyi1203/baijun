<?php
class clsArticleEvaluationAddController extends clsAppController implements IAction_default , IAction_insert{
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        $input->objecttype  = "evaluation";
        $input->public      = "0";

        $model  = new clsModModel($this->mdb ,'mw_article');
        if (!$model->mw_article->insert($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->evaluation->failinsert;
            return;
        }
        ...
        if(!$this->loadController('admin.system.file.default')
             ->updateObjectID($this->input->uid, $insertid, 'evaluation')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->evaluation->failinsert;
            return;
        }
        ...
        $this->output->message = $this->lang->evaluation->successinsert;
        $this->output->locate  = 'article_evaluation-default-default.html';
