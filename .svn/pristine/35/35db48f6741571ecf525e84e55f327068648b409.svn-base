<?php
class clsGroupLevelAddController extends clsAppController implements IAction_default , IAction_insert{
    ...
    public function _insert() {
        ...
        if (!$model->mw_category->insert($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->level->failinsert;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->level->successinsert;
