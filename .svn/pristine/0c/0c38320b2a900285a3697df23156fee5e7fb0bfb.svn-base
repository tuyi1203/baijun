<?php
class clsExampleRoomAddController extends clsAppController implements IAction_default , IAction_insert{
    ...
    public function _insert() {
        ...
        if (!$model->mw_category->insert($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->room->failinsert;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->room->successinsert;
