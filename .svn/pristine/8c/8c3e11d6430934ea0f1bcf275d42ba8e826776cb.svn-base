<?php
class clsCultureMetropolitanDefaultController extends clsAppController
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        if(!$model->mw_article->delete($this->input)) {
            ...
            $this->output->message = $this->lang->metropolitan->faildelete;
            return;
        }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->metropolitan->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    private function init() {
        ...
        if (isset($_SESSION['action_auth_list']['admin']['culture/metropolitan/default/publish']) and
                $_SESSION['action_auth_list']['admin']['culture/metropolitan/default/publish']['operauth'] == '1') {
