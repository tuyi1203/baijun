<?php
class clsCustomerClaimEditController extends clsAppController implements IAction_default , IAction_update , IAction_publish{
    ...
    public function _update() {
        ...
        if (!$model->mw_message->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->claim->failupdate;
            return;
        }

//         if(!$this->loadController('admin.system.file.default')
//                  ->updateObjectID($this->input->uid, $this->input->id, 'claim')) {
//             $this->output->result  = 'fail';
//             $this->output->message = $this->lang->claim->failupdate;
//             return;
//         }
        ...
        $this->output->message = $this->lang->claim->successupdate;
        $this->output->locate  = U('customer/claim');
