<?php
class clsSingleIntroDefaultController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_single->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->intro->failupdate;
            return;
        }

        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'intro')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->intro->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->intro->successupdate;
        $this->output->locate  = 'single_intro-default-default.html';
