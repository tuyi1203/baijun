<?php
class clsHomeTopcaseAddController extends clsAppController implements IAction_default , IAction_insert{
    ...
    public function _insert() {
        ...
        if($ok) {
            ...
            $this->output->message = $this->lang->topcase->successinsert;
            $this->output->locate  = "home_topcase-default-default-category-{$input->category}.html";
        } else {
            $this->mdb->subRollBack();
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->topcase->failinsert;
        }
    }
