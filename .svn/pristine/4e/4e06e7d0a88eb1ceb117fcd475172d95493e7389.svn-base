<?php
class clsNewsBrotherhoodDefaultController extends clsAppController
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        if(!$model->mw_article->delete($this->input)) {
            ...
            $this->output->message = $this->lang->brotherhood->faildelete;
            return;
        }
    ...
    public function _settop(){
        ...
        if(!$model->mw_article->toggletop($this->input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->brotherhood->failsettop;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->brotherhood->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    private function init() {
        ...
        if (isset($_SESSION['action_auth_list']['admin']['news/brotherhood/default/publish']) and
                $_SESSION['action_auth_list']['admin']['news/brotherhood/default/publish']['operauth'] == '1') {
            $this->output->publishauth = '1';
        }
        if (isset($_SESSION['action_auth_list']['admin']['news/brotherhood/default/settop']) and
        		$_SESSION['action_auth_list']['admin']['news/brotherhood/default/settop']['operauth'] == '1') {
