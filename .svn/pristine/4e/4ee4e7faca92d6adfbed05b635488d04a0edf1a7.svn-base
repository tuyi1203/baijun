<?php
class clsSingleEventDefaultController extends clsAppController
...
//         var_dump($this->cookie->lang);
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->event->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    private function init() {
        ...
        if (isset($_SESSION['action_auth_list']['admin']['single/event/default/publish']) and
                $_SESSION['action_auth_list']['admin']['single/event/default/publish']['operauth'] == '1') {
