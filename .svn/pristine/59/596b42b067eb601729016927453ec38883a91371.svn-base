<?php
class clsInformationWaterpressureDefaultController extends clsAppController
...
// pr($_SESSION);
        ...
        $input = new stdClass();
        $input->objecttype = "waterpressure";
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        if(!$model->mw_article->delete($this->input)) {
            ...
            $this->output->message = $this->lang->waterpressure->faildelete;
            return;
        }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->waterpressure->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    private function init() {
        ...
        if (isset($_SESSION['action_auth_list']['admin']['information/waterpressure/default/publish']) and
                $_SESSION['action_auth_list']['admin']['information/waterpressure/default/publish']['operauth'] == '1') {
