...
function getModeOptions() {
    return array('admin'=>'admin' , 'site'=>'site' , 'wxadmin'=>'wxadmin' , 'wxsite'=>'wxsite' , 'wechat'=>'wechat','addon'=>'addon');
}
...
function makeOptionsForAjaxRequest($keyValueArray) {
    $options = array();
...
//取得验证码设置值
function getCaptchaSet() {
    $result =  getMwSet('captcha', '1');
    return $result[0]['value'];
}

function getMwSet($key , $subkey) {
//     if(!class_exists('Mw_Set')) require APATH_LIBS_MODELS . DS .'mw_set.php';
    $model = new Mw_Set(getMDB(), 'Mw_Set');
    $input = new stdClass();
    $input->key = $key;
    $input->subkey = $subkey;
    $value = $model->get($input);
    return $value;
...
//取得数据库中保存的accesstoken
function getToken() {
    $dao = getDAO();
    $row = $dao->select('*')->from('wc_set')
                ->where('`key`')->eq('wechatsetting')
                ->andWhere('subkey')->eq('accesstoken')
                ->fetch();
    return $row->value;
}
...
function getTokenExpireTime()
{
    ...
    $row = $dao->select('*')->from('wc_set')
                ->where('`key`')->eq('wechatsetting')
                ->andWhere('subkey')->eq('accesstokenexpiretime')
                ->fetch();
    return $row->value;
}
...
function updateToken($accesstoken)
{
    $dao = getDAO();
    $dao->begin();
    $dao->update('wc_set')->set('value')->eq($accesstoken['access_token'])
            ->where('`key`')->eq('wechatsetting')
            ->andWhere('subkey')->eq('accesstoken')
            ->exec();
    $dao->update('wc_set')->set('value')->eq($accesstoken['expiretime'])
            ->where('`key`')->eq('wechatsetting')
            ->andWhere('subkey')->eq('accesstokenexpiretime')
            ->exec();
    $dao->commit();
