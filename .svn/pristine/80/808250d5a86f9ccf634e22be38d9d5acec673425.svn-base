<?php
class clsGroupLevelDefaultController extends clsAppController
...
// pr($this->input->sort);
        ...
        if ($ok) {
            ...
            $this->output->message = $this->lang->level->successsort;
        }
        else {
            $this->mdb->subRollBack();
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->level->failsort;
        }
