<?php
class clsExampleCaseEditController extends clsAppController implements IAction_default , IAction_update{
...
// pr($_POST);exit;
        ...
        if (!$model->yjm_case->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->case->failupdate;
            return;
        }

        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'case')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->case->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->case->successupdate;
    ...
    private function init() {
    	$model = new clsModModel($this->mdb, 'yjm_case');
    	$person = $model->yjm_case->get($this->input);
