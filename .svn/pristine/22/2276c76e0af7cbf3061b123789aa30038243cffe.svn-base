<?php
class clsSingleListEditController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'single')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->list->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->list->successupdate;
    ...
    private function init() {
        $model = new clsModModel($this->mdb , 'mw_single');
        $output = $model->mw_single->get($this->input);
