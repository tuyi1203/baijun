<?php
class clsSingleTechnologyDefaultController extends clsAppController implements IAction_default , IAction_update{
...
//         $input->content2    = $this->input->content2;
        ...
        if (!$model->mw_single->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->technology->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->technology->successupdate;
        $this->output->locate  = 'single_technology-default-default.html';
    ...
    private function init() {
        ...
        $this->output->editor         = array('id' => array('content','content2'), 'tools' => 'simple');
    }
