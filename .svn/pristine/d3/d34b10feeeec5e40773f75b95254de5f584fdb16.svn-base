<?php
class clsCustomerMessageEditController extends clsAppController implements IAction_default , IAction_update , IAction_publish{
    ...
    public function _update() {
        ...
        if (!$model->mw_message->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->message->failupdate;
            return;
        }

//         if(!$this->loadController('admin.system.file.default')
//                  ->updateObjectID($this->input->uid, $this->input->id, 'message')) {
//             $this->output->result  = 'fail';
//             $this->output->message = $this->lang->message->failupdate;
//             return;
//         }
        ...
        $this->output->message = $this->lang->message->successupdate;
        $this->output->locate  = U('customer/message');
