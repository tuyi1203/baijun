<?php
class clsArticleReviewAddController extends clsAppController implements IAction_default , IAction_insert{
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if (!$model->mw_article->insert($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->review->failinsert;
            return;
        }
        ...
        if(!$this->loadController('admin.system.file.default')
             ->updateObjectID($this->input->uid, $insertid, 'review')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->review->failinsert;
            return;
        }
        ...
        $this->output->message = $this->lang->review->successinsert;
        $this->output->locate  = 'article_review-default-default.html';
