<?php
class clsCustomerAccusationDefaultController extends clsAppController
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        $model = new clsModModel($this->mdb,'mw_message');
        if(!$model->mw_message->delete($this->input)) {
            $this->output->result  = 'fail';
            $this->output->accusation = $this->lang->message->faildelete;
            return;
        }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->accusation = $this->lang->message->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    {
        $model = new clsModModel($this->mdb,'mw_message');
        if(!$model->mw_message->publish($this->input)) {
            return false;
        }
        return true;
    }
    ...
    private function init() {
        ...
        if (!isset($this->input->orderby)) {
            ...
            $this->input->orderby = in_array($this->input->orderby , array('createtime','public' , 'replyid'))?$this->input->orderby:"createtime";
        ...
        if (isset($_SESSION['action_auth_list']['admin']['customer/accusation/default/publish']) and
                $_SESSION['action_auth_list']['admin']['customer/accusation/default/publish']['operauth'] == '1') {
