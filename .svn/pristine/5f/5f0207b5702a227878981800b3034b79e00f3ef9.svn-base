<?php
class clsWebsiteAnnounceEditController extends clsAppController implements IAction_default , IAction_update , IAction_publish{
...
//         $input->public      = "0";
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'announce')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->announce->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->announce->successupdate;
