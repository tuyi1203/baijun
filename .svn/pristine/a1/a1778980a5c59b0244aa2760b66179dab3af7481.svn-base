<?php
class Mw_Smallslides extends clsModel{

    CONST INSERT     = "insert into mw_smallslides (title,link,label,`desc`,createby,createtime)values (?,?,?,?,?,?)";

    CONST GETLIST    = "select a.* , b.url , b.`primary` from mw_smallslides a left join (select url , objectid ,`primary` from mw_file b where objecttype=? and editor='0') b on a.id = b.objectid order by sort asc";

    CONST GETCOUNT   = "select count(*) cnt from mw_smallslides";

    CONST GET        = "select a.* , b.url , b.id fileid from mw_smallslides a left join (select id , url , objectid from mw_file b where objecttype=? and objectid=? and editor='0') b on a.id = b.objectid where a.id=? order by a.sort asc";

    CONST UPDATE     = "update mw_smallslides set title=? , link=? , label=?,`desc`=?,updateby=?,updatetime=? where id=?";

    CONST DELETE     = "delete from mw_smallslides where id = ?";

    CONST UPDATE_SORT  = "update mw_smallslides set sort=? where id = ?";
