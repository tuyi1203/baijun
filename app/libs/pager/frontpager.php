...
class frontpager {
    ...
    private function createPrevPage() {
        ...
        if ($this->currPage != 1) {
            return $this->createSpecLink($this->lang->link->prevpage , $this->prevPage);
        }

        return $this->createSpecLink($this->lang->link->prevpage);
    }

    private function createNextPage() {
        if ($this->currPage != $this->pageCount) {
            return $this->createSpecLink($this->lang->link->nextpage , $this->nextPage);
        }

        return $this->createSpecLink($this->lang->link->nextpage);
    }
...
//         $goToHtml .= "<input type='hidden' id='_pageTotal' value='$this->pageTotal' />\n";
    ...
    private function createTotal() {
        return sprintf($this->lang->link->total , $this->currPage , $this->pageCount);
    }
    ...
    private function createRecPerPageJS()
    {
        ...
        function submitPage(mode)
        {
            ...
            location.href = '$this->modulef' + '_' + '$this->modules' + '-' + '$this->script' + '-paging-currpage-' + pageID + '.html';
        }
        </script>
EOT;
        return $js;
    ...
    private function createRange() {
        ...
        for ($i = 1 ; $i < $this->currPage; $i ++) {
            $l_aStackFont[] = $this->createLink($i,$i);
        }

        for ($i = $this->currPage + 1 ; $i < $this->pageCount + 1; $i ++) {
            $l_aStackBack[] = $this->createLink($i,$i);
        }
        ...
        $html[] = $this->createLink($this->currPage,$this->currPage,true);
