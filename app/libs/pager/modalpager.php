...
class modalPager {
    ...
    public function __construct($recTotal = 0, $recPerPage = 20, $pageID = 1 , $paramstr=null , $orderby=null , $sort=null) {
    ...
    private function createNextPage() {
        if ($this->currPage != $this->pageCount) {
            return $this->createLink($this->lang->link->nextpage , $this->nextPage);
        }

        return $this->createLink($this->lang->link->nextpage);
    }
    ...
    private function createLink($label , $pageid = 0) {
//         echo $label;
        if (!$pageid) return $label;
        if (!empty($this->sort) and !empty($this->orderby))
//             return '<a href="' . U($this->modulef . '/' . $this->modules . '/' . $this->script . '/paging/currpage/'. $pageid .'/orderby/'.$this->orderby.'/sort/'. $this->sort .'.html') . '">'.$label.'</a>';
        		return '<a href="javascript:void(0);" onclick="paging(\'' . U($this->modulef . '/' . $this->modules . '/' . $this->script . '/paging/currpage/'. $pageid .'/orderby/'.$this->orderby.'/sort/'. $this->sort . '/' . $this->linkParams . APPTYPE) . '\');">'.$label.'</a>';
        return '<a href="javascript:void(0);" onclick="paging(\'' . U($this->modulef . '/' . $this->modules . '/' . $this->script . '/paging/currpage/'. $pageid . '/' . $this->linkParams . APPTYPE) . '\');">'.$label.'</a>';
...
//         $goToHtml .= "<input type='hidden' id='_pageTotal' value='$this->pageTotal' />\n";
        ...
        $goToHtml .= "<input type='button' id='goto'       value='{$this->lang->link->locate}' onclick='submitPage();' class='btn btn-default btn-xs' />";
        return $goToHtml;
    }


    private function createTotal() {
        return sprintf($this->lang->link->total , $this->recTotal);
    }
    ...
    {
        $url = U($this->modulef . '/' . $this->modules . '/' . $this->script . '/paging/' . $this->linkParams . '/currpage/' , '' , false );
        $js  = <<<EOT
        ...
        function submitPage()
        {
            ...
            paging('$url' + pageID + '$this->apptype');
        }
        function paging(url)
        {
			$.reloadAjaxModalFromUrl(url);
			return false;
        }
        </script>
EOT;
...
//     if (count($l_aStackFont) + count($l_aStackBack) + 1 > LINKRANGE) {
//         if (count($l_aStackBack) < (int)(LINKRANGE/2)) {
//             while(count($l_aStackFont) + count($l_aStackBack) + 1 != LINKRANGE) {
//                 array_shift($l_aStackFont);
//             }
//         } else if (count($l_aStackFont) < (int)(LINKRANGE/2)) {
//             while(count($l_aStackFont) + count($l_aStackBack) + 1 != LINKRANGE) {
//                 array_pop($l_aStackBack);
//             }
//         } else {
//             while(count($l_aStackFont) != (int)(LINKRANGE/2)) {
//                 array_shift($l_aStackFont);
//             }
//             if (LINKRANGE & 1) {//奇数时
//                 while(count($l_aStackBack) != (int)(LINKRANGE/2)) {
//                     array_pop($l_aStackBack);
//                 }
//             } else {
//                 while(count($l_aStackBack) != (int)(LINKRANGE/2) + 1){
//                     array_pop($l_aStackBack);
//                 }
//             }
//         }
