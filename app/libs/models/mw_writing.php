<?php
class mw_writing extends clsModel{

    CONST INSERT     = "insert into mw_writing (title,`desc`,sort,createby,createtime,publishyear , homepage )values (?,?,?,?,?,?,?)";

    CONST GETLIST    = "select * from mw_writing order by publishyear desc , sort desc ";

    CONST GETCOUNT   = "select count(*) cnt from mw_writing";

    CONST GET        = "select * from mw_writing where id=?";

    CONST UPDATE     = "update mw_writing set title=? , `desc`=? , sort=?,updateby=? , updatetime=? , publishyear=? , homepage=?  where id=?";

    CONST DELETE     = "delete from mw_writing where id =?";

    CONST GETTOPLIST = "select a.* , b.url from ( select * from mw_writing where homepage='1') a left join (select url , id from mw_topimage where objecttype=? ) b on a.id = b.objectid where a.subkey =? order by sort ";

    CONST GETHOMEPAGELIST = "select * from mw_writing where subkey =? and homepage = '1' order by sort ";
