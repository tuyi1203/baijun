<?php
class Wc_File extends clsModel{

    CONST INSERT = "insert into wc_file (title , filename , filemd5name , filepath , url , ext , mimetype , size , width , height , createby , createtime , public , `primary` , editor , objecttype , objectid) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

    CONST UPDATEOBJECT = "update wc_file set objectid = ? , objecttype=? where id in(?)";

    CONST GETLIST = "select a.* , b.name from wc_file a left join eku_user_info b on a.createby = b.uid where a.objecttype=? and a.objectid = ? and a.editor <> '1' order by a.`primary` desc , a.id asc";

    CONST SETPRIMARY = "update wc_file set `primary`=?";

    CONST DELETE = "delete from wc_file where id in (?)";

    CONST DELETEBYOBJECT = 'delete from wc_file where objecttype=? and objectid=?';

    CONST GETBYID = "select * from wc_file where id=?";

    CONST UPDATETITLE = "update wc_file set title=? where id=?";

    CONST UPDATEFILE = "update wc_file set title=? ,filename =?,filemd5name =?,filepath =?,url =?,ext =?,mimetype =?,size =?,width =?,height =?,updateby =?,updatetime =? where id=?";
