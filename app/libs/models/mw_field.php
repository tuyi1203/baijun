<?php
class Mw_Field extends clsModel{

    CONST INSERT     = "insert into mw_field (title,entitle,link,label,`desc`,createby,createtime)values (?,?,?,?,?,?,?)";

    CONST GETLIST    = "select a.* , b.url , b.`primary` from mw_field a left join (select url , objectid ,`primary` from mw_file b where objecttype=? and editor='0') b on a.id = b.objectid where a.pid is null order by sort asc";

    CONST GETCOUNT   = "select count(*) cnt from mw_field where pid is null";

    CONST GET        = "select a.* , b.url , b.id fileid from mw_field a left join (select id , url , objectid from mw_file b where objecttype=? and objectid=? and editor='0') b on a.id = b.objectid where a.id=? order by a.sort asc";

    CONST UPDATE     = "update mw_field set title=? , entitle=?, link=? , label=?,`desc`=?,updateby=?,updatetime=? where id=?";

    CONST DELETE     = "delete from mw_field where id = ?";

    CONST UPDATE_SORT  = "update mw_field set sort=? where id = ?";
