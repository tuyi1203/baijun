<?php
class Mw_Single extends clsModel{

    CONST INSERT     = "insert into mw_single (title,keyword,summary,content,publishtime,createby,createtime,status )values (?,?,?,?,?,?,?,?)";

    CONST GETLIST    = "select * from mw_single";

    CONST GETCOUNT   = "select count(*) cnt from mw_single";

    CONST GET        = "select * from mw_single where id=?";

    CONST UPDATE     = "update mw_single set title=? , keyword=? , summary=? , content=? ,content2=?, publishtime=? , status=? , updateby=? , updatetime=? where id=?";

    CONST DELETE     = "delete from mw_single where id =?";

    CONST VIEWPLUS    = "update mw_single set views=views+1 where id=?";
