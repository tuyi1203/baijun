<?php
class clsKnowledgeQuestionEditController extends clsAppController implements IAction_default , IAction_update {
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if (!$model->mw_question->update($input)) {
            $this->output->result  = 'fail';
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'question')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->question->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->question->successupdate;
    ...
    private function init() {
        $model = new clsModModel($this->mdb , 'mw_question');
        $output = $model->mw_question->get($this->input);
        foreach ($output as $k => $v) {
            $this->output->$k = $v;
            if ($k == "status") $this->output->status_choose = $v;
            if ($k == "category") $this->output->category_choose = $v;
        }
