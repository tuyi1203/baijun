<?php
class clsArticleJoinEditController extends clsAppController implements IAction_default , IAction_update , IAction_publish{
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'join')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->join->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->join->successupdate;
        $this->output->locate  = 'article_join-default-default.html';
