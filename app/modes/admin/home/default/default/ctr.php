...
class clsHomeDefaultDefaultController extends clsAppController
    ...
    public function _default() {
        ...
        //
        $arr = array();
        $ekumodel = new Eku_Module($this->mdb , "Eku_Module");
        $models = $ekumodel->queryModelByPublish($this->session->getUid());

        $articlemodel = new Mw_Article($this->mdb , "mw_article");
        $messagemodel = new Mw_Message($this->mdb , "mw_message");
        //$questionmodel = new mw_question($this->mdb , "mw_question");
        foreach ($models as $key => $value)
        {
            if ($value['parentname'] == 'customer' || $value['parentname'] == 'service')
            {
                $count = $messagemodel->queryCountByPublish($value['name']);
            }
            else if($value['parentname'] == 'knowledge')
            {
                 //$count = $questionmodel->queryCountByPublish();
            }
            else
            {
                $count = $articlemodel->queryCountByPublish($value['name']);
            }
            ...
            $url = "/index.php/admin/{$value['parentname']}/{$value['name']}/default/default.html";
            array_push($arr, array('title'=>$value['des'], 'count'=>$count, 'url'=>$url));
        }

        $this->output->models = $arr;
        if ($recTotal > 0) {
