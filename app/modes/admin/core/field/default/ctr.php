<?php
class clsCoreFieldDefaultController extends clsAppController
...
// pr($_SESSION['menu_list']);
        ...
        $recTotal = $model->mw_field->getCount($this->input);

        if ($recTotal > 0) {
            $input = new stdClass();
            $input->objecttype = "field";
            $result = $model->mw_field->getList($input);
...
// pr($this->input->sort);
        ...
        if ($ok) {
            ...
            $this->output->message = $this->lang->field->successsort;
        }
        else {
            $this->mdb->subRollBack();
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->field->failsort;
        }
    ...
    public function _delete() {
       ...
       if(!$model->mw_field->delete($this->input)) {
        ...
        if($ok) {
            ...
            $this->mdb->subRollBack();
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->field->faildelete;
        }
