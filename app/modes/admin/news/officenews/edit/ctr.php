<?php
class clsNewsOfficenewsEditController extends clsAppController implements IAction_default , IAction_update {
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'officenews')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->officenews->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->officenews->successupdate;
        $this->output->locate  = U('news/officenews/default/default.html');
