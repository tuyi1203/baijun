<?php
class clsWebsiteSlidesEditController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_slides->update($input))  $ok = false;
//pr($this->input->fileids);
        ...
        if(!$this->loadController('admin.system.file.default')
                ->updateObjectID($this->input->uid, $this->input->id, 'slides')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->slides->failupdate;
            return;
        }
        ...
        if($ok) {
            ...
            $this->output->message = $this->lang->slides->successupdate;
    ...
    private function init() {
        ...
        $model = new clsModModel($this->mdb, 'mw_slides');
        $slide = $model->mw_slides->getByID($input);
