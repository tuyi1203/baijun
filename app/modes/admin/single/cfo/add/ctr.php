<?php
class clsSinglecfoAddController extends clsAppController implements IAction_default , IAction_insert{
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if (!$model->mw_article->insert($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->cfo->failinsert;
            return;
        }
        ...
        if(!$this->loadController('admin.system.file.default')
             ->updateObjectID($this->input->uid, $insertid, 'cfo')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->cfo->failinsert;
            return;
        }
        ...
        $this->output->message = $this->lang->cfo->successinsert;
        $this->output->locate  = U('single/cfo/default/default.html');
