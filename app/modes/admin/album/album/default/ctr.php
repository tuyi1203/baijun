<?php
class clsAlbumAlbumDefaultController extends clsAppController
...
// 		$this->input->key = 'album';
		...
		$recTotal = $model->mw_album->getCount($this->input);
		...
		if ($recTotal > 0) {
			...
			$result = $model->mw_album->getList($input);
    ...
    public function _delete() {
        ...
        $model = new clsModModel($this->mdb,'mw_album,mw_file');
        if(!$model->mw_album->delete($this->input)) {
