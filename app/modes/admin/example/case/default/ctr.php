<?php
class clsExampleCaseDefaultController extends clsAppController
...
// pr($_SESSION);
        ...
        if ($recTotal > 0) {
            ...
            $result = $model->yjm_case->getList($input);
...
//         pr($_SESSION);
    ...
    public function _delete() {
        ...
        $model = new clsModModel($this->mdb,'eku_user,yjm_case,mw_file');
        $ok = true;
...
//         }
        ...
        if( $ok and !$model->yjm_case->delete($this->input)) {
        	$ok = false;
        }
    ...
    private function init() {
        ...
        if (!isset($this->input->orderby)) {
            ...
            $this->input->orderby = in_array($this->input->orderby , array('sort','style','room','surface','designer','views','homepage'))?$this->input->orderby:"sort";
