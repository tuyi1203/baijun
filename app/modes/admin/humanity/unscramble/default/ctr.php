<?php
class clsHumanityUnscrambleDefaultController extends clsAppController
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        if(!$model->mw_world->delete($this->input)) {
            ...
            $this->output->message = $this->lang->unscramble->faildelete;
            return;
        }
    ...
    public function _settop(){
        ...
        if(!$model->mw_world->toggletop($this->input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->unscramble->failsettop;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->unscramble->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    private function init() {
        ...
        if (isset($_SESSION['action_auth_list']['admin']['humanity/unscramble/default/publish']) and
                $_SESSION['action_auth_list']['admin']['humanity/unscramble/default/publish']['operauth'] == '1') {
            $this->output->publishauth = '1';
        }
        if (isset($_SESSION['action_auth_list']['admin']['humanity/unscramble/default/settop']) and
        		$_SESSION['action_auth_list']['admin']['humanity/unscramble/default/settop']['operauth'] == '1') {
