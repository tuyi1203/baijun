...
// 		$this->input->key = 'album';
		...
		$recTotal = $model->mw_writing->getCount($this->input);
		...
		if ($recTotal > 0) {
			...
			$result = $model->mw_writing->getList($input);
    ...
    public function _delete() {
        ...
        $model = new clsModModel($this->mdb,'mw_writing,mw_file');
        if(!$model->mw_writing->delete($this->input)) {
