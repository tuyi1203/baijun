<?php
class clsHumanitylifeEditController extends clsAppController implements IAction_default , IAction_update {
...
//         if (isset($this->input->lawyer)) $input->lawyer      = implode(',', $this->input->lawyer);
        ...
        $input->status      = $this->input->status;
        $input->views       = $this->input->views;
        $input->lawyerflg   = $this->input->lawyerflg;
        $input->id          = $this->input->id;
        if ($this->input->lawyerflg == '0') {
            $input->authorname = $this->input->authorname;
            $input->authorid   = -1;
        } else {
            $input->authorname = '';
        ...
        if (!$this->model->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->life->failupdate;
            return;
        }
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'life')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->life->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->life->successupdate;
        $this->output->locate  = U('humanity/life/default/default.html');
    ...
    private function init() {
        ...
        foreach ($output as $k => $v) {
            ...
            if ($k == "lawyerflg") $this->output->author_choose = $v;
        }

        if ($this->output->lawyerflg == '1') {
        	$this->output->lawyerauthorname = $this->model->getAuthorName($this->output->authorid);
        } else {
            $this->output->authorname = $this->output->authorname;
        }
