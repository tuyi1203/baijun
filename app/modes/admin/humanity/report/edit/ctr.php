<?php
class clsHumanityReportEditController extends clsAppController implements IAction_default , IAction_update {
...
//         $input->publishtime = $this->input->publishtime == "0000-00-00 00:00:00" ? date("Y-m-d H:i:s") : $this->input->publishtime;
        ...
        if (!$model->mw_report->update($input)) {
            $this->output->result  = 'fail';
        ...
        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'report')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->report->failupdate;
            return;
        }

        $this->output->result  = 'success';
        $this->output->message = $this->lang->report->successupdate;
    ...
    private function init() {
        $model = new clsModModel($this->mdb , 'mw_report');
//         $output = $model->mw_report->get($this->input);
