<?php
class clsHumanityReportDefaultController extends clsAppController
...
// pr($_SESSION);
        $this->init();
        $model = new clsModModel($this->mdb , "mw_report");
        $input = new stdClass();
        $input->objecttype = "report";
        $recTotal = $model->mw_report->getCount($input);
...
//             $input  = new stdClass();
            ...
            $result = $model->mw_report->getList($input);
// pr($result);
...
//         var_dump($this->cookie->lang);
    ...
    public function _delete() {
        ...
        $model = new clsModModel($this->mdb,'mw_report');
        if(!$model->mw_report->delete($this->input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->report->faildelete;
            return;
        }
    ...
    public function _settop(){
        $model = new clsModModel($this->mdb,'mw_report');
        if(!$model->mw_report->toggletop($this->input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->report->failsettop;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    public function _publish() {
        ...
        if(!$this->publish()) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->report->failpublish;
            return;
        }
        $this->output->result  = 'success';
    }
    ...
    public function publish() {
        $model = new clsModModel($this->mdb,'mw_report');
        if(!$model->mw_report->publish($this->input)) {
            return false;
        }
        return true;
    }
    ...
    private function init() {
        ...
        if (!isset($this->input->orderby)) {
            ...
            $this->input->orderby = in_array($this->input->orderby , array('id','title','createtime','views','public','createby','status','period'))?$this->input->orderby:"id";
        ...
        if (isset($_SESSION['action_auth_list']['admin']['humanity/report/default/publish']) and
                $_SESSION['action_auth_list']['admin']['humanity/report/default/publish']['operauth'] == '1') {
            $this->output->publishauth = '1';
        }
        if (isset($_SESSION['action_auth_list']['admin']['humanity/report/default/settop']) and
        		$_SESSION['action_auth_list']['admin']['humanity/report/default/settop']['operauth'] == '1') {
