...
function lawyerselect(id,name) {
	...
	$('.nolawyer').remove();
	$('.zc_tags').append('<label onclick="deletelawyer(this);" data-id="'+id+'">'+name+' <span>×</span><input type="hidden" name="data[author][]" value="'+id+'"/></label>');
...
function deletelawyer(obj) {
	...
	if ($('.zc_tags').find('label').length == 0 ) {
		$('.zc_tags').append('<label class="nolawyer">'+noauthor+'</label>');
	}
}