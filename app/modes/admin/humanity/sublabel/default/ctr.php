<?php
class clsHumanitySublabelDefaultController extends clsAppController
...
//         $this->mdb->subSetAutoCommit(true);//打开自动提交
        ...
        if ($this->model->saveSort($this->input)) {
            $this->output->result  = 'success';
            $this->output->message = $this->lang->sublabel->successsort;
        } else {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->sublabel->failsort;
        }
    ...
    private function init() {
        $pidoptions = $this->model->getLabels();
        if (count($pidoptions) < 1) {
            $this->redirect(U('humanity/label/default'));
        }
