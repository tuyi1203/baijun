<?php
class clsSystemChatfileDefaultController extends clsAppController
            implements IAction_default , IAction_upload{
    ...
    public function _default()
    {
		...
		if ($recTotal > 0) {
			...
			if ($this->input->filetype == "image") {
				$recperpage = 15;//每页显示15条数据
				$pager = new modalPager($recTotal , $recperpage , $currPage = 1 , 'filetype/'.$this->input->filetype);
			}
			$this->input->start = $pager->getRecStart();
			$this->input->end   = $pager->getRecEnd();
			$list = obj2array($this->model->getList($this->input));
        ...
        $this->output->filetype  = $this->input->filetype;
    }
    ...
    public function _paging()
    {
    	...
    	if ($recTotal > 0) {
    		...
    		if ($this->input->filetype == "image") {
    			$recperpage = 15;//每页显示15条数据
    			$pager = new modalPager($recTotal , $recperpage , $this->input->currpage , 'filetype/'.$this->input->filetype);
    		}
    		$this->input->start = $pager->getRecStart();
    		$this->input->end   = $pager->getRecEnd();
    		$list = obj2array($this->model->getList($this->input));
    ...
    public function _upload()
    {
        ...
        //一致性检查
//         pr($this->post);
        $verifyToken = md5(C('md5salt') . $this->input->timestamp);

        if ($this->input->token != $verifyToken) {
            $this->output->result  = "fail";
            $this->output->message = $this->lang->chatfile->unsafe;
            return;
        }
        ...
        $files = $this->getUpload('Filedata');
//TODO 错误处理
        if($files) {
//             pr($files);
            if (! $this->saveUpload($this->input->objecttype, $this->model->getObjectID($this->input) , $files)) {
                $this->output->result  = "fail";
    ...
    public function _download()
    {
        ...
        $file = $model->wc_file->getById($this->input);
...
//             $this->app->triggerError("The file you visit $fileID not found.", __FILE__, __LINE__, true);
    ...
    public function deleteFile($id) {
        ...
        $input->id = $id;
        $model = new clsModModel($this->mdb, 'wc_file');
        $file = $model->wc_file->getByID($input);

        if(file_exists($file['filepath'])) unlink($file['filepath']);

        if(!$model->wc_file->delete($input)) return false;
        return true;
    ...
    public function _primary() {
        ...
        $input->primary    = '0';
        if(!$model->wc_file->setPrimary($input)) {
            $this->output->result  = 'fail';
...
//         pr($file);
        if(!isset($file['errmsg'])) {
            ...
            $model = new clsModModel($this->mdb, 'wc_file');
            $model->wc_file->insert($input);
            $_SESSION['album'][$this->input->uid][] = $model->wc_file->lastInsertID();
...
//             $this->compressImage($this->savePath . $file['pathname']);
    ...
    public function updateObjectID($uid, $objectID, $objectType)
    {
        ...
        if(isset($_SESSION['album'][$uid]) && $_SESSION['album'][$uid])
        {
            ...
            $model = new clsModModel($this->mdb, 'wc_file');
            return $model->wc_file->updateObject($input);
        }
        return true;
    }
