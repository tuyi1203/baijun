...
class clsModModel extends clsAppModel{
	...
	public function getLawyer($input)
	{
		$row = $this->dao->select()->from("mw_lawyer")->where("id")->eq($input->id)
						->fetch();
		return $row;
	}
	...
	{
		$row = $this->dao->select()->from("mw_field")->where("id")->eq($id)
						->fetch();
		return $row;
	}
	...
	public function getOfficenewsList($input)
	{
		...
		$rows = $this->dao->select()->from("mw_article")->where("objecttype")->eq($input->objecttype)
							->andWhere("public")->eq($input->public)->andWhere("find_in_set('$input->lawyerid',lawyer)")
							->orderby("publishtime desc")->limit($input->start , $input->end)->fetchAll();
//		pr($rows);
		if (!empty($rows)) {
	...
	public function getPerformancelist($input)
	{
		...
		$rows = $this->dao->select()->from("mw_article")->where("objecttype")->eq($input->objecttype)
			...
			->andWhere("find_in_set('$input->lawyerid',lawyer)")
