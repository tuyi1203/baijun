<?php
class clsSingleAllDefaultController extends clsAppController implements IAction_default , IAction_update{
    ...
    public function _update() {
        ...
        if (!$model->mw_single->update($input)) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->all->failupdate;
            return;
        }

        if(!$this->loadController('admin.system.file.default')
                 ->updateObjectID($this->input->uid, $this->input->id, 'all')) {
            $this->output->result  = 'fail';
            $this->output->message = $this->lang->all->failupdate;
            return;
        }
        ...
        $this->output->message = $this->lang->all->successupdate;
        $this->output->locate  = 'single_all-default-default.html';
