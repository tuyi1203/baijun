...
class dao
{
     ...
     * @return void
     */
    public function __construct($dbh , $slaveDBH = false)
    {
        $this->dbh      = $dbh;
        $this->slaveDBH = $slaveDBH;
        $this->reset();
    }
    ...
    public function prepare($sql , array $driver_options = array())
    {
		...
		$stmt = new statement($this->dbh , $this);
		return $stmt->prepare($sql , $driver_options);
    }
    ...
    private function processSQL()
    {
        ...
        if($this->mode == 'magic')
        {
            ...
            if($this->table == '')  throw new clsDbException('表名不能为空');
    ...
    public function query()
    {
        ...
        {
            $this->isFail = true;
            clsLogger::subWriteDbError('SQL実行失敗' . PHP_EOL . "\t" . $e->getCode() . ": " .$e->getMessage() . PHP_EOL . "\t" .  $sql);
            if ($this->inTran()) $this->rollBack();
        }
    }
...
class sql
{
    ...
    public function data($data, $skipFields = '')
    {
        ...
        foreach($data as $field => $value)
        {
            ...
            $this->sql .= "`$field` = " . (is_null($value)?'null':$this->quote($value)) . ',';
        }
...
class statement {
    ...
    public function __construct($dbh , $dao)
    {
        $this->dbh = $dbh;
        $this->dao = $dao;
    }
    ...
    public function prepare($sql , array $driver_options = array())
    {
        $this->stmt = $this->dbh->prepare($sql , $driver_options);
    ...
    public function __call($funcName, $funcArgs)
    {
        return call_user_method_array(array($this->stmt, $funcName), $funcArgs);
    }
}
